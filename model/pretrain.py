...
def pretrain_vec(data_name, bag_size=20, size=64, neg=5, window=5, core=0):
    """pretain word2vec
    ...
    """
    if os.path.exists(get_vec_filename(data_name, 'i', bag_size, size, neg, window)):
    ...
    data.insert(0, ['<UNK>'])
    model = Word2Vec(sentences=data, size=size,
                     window=5, min_count=1, workers=4, negative=5)
    model.save(
